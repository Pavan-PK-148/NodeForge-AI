...
const LightPillar = ({
  ...
  if (!webGLSupported) {
    return (
      <div
        className={`w-full h-full absolute top-0 left-0 flex items-center justify-center bg-black/40 text-slate-500 text-xs ${className}`}
        style={{ mixBlendMode }}
      >
        WebGL Unvailable
      </div>
...
export default function ThreeBackground({ isScraping }) {
  return (
    <div className="absolute inset-0 w-full h-full pointer-events-none z-0 overflow-hidden bg-[#020105]">
      {/* Dynamic base layer pillar */}
      <div className="absolute inset-0 w-full h-full opacity-60">
        <LightPillar
          topColor={isScraping ? "#00F2FE" : "#4facfe"}
          bottomColor={isScraping ? "#4FACFE" : "#000000"}
          intensity={isScraping ? 1.6 : 1.1}
          rotationSpeed={isScraping ? 0.9 : 0.25}
          glowAmount={0.003}
          pillarWidth={3.5}
          pillarHeight={0.35}
          noiseIntensity={0.3}
      ...
      {/* Counter ambient accent layer pillar */}
      <div className="absolute inset-0 w-full h-full opacity-45 transform scale-x-[-1]">
        <LightPillar
          topColor="#250059"
          bottomColor="#f35588"
          intensity={0.8}
      ...
      {/* Matte Contrast overlay prevents blinding text */}
      <div className="absolute inset-0 bg-gradient-to-t from-[#030206] via-[#030206]/40 to-[#030206]" />
